...
public class V3Language implements Language {
    ...
    public void initializeTypeEnv(Program program, Cache<Type> typeCache, TypeEnv env) {
        ...
        VirgilArray.IType stringType = VirgilArray.TYPECON.newType(typeCache, PrimChar.TYPE);
        env.bindTypeCon("string", new TypeCon.Singleton(stringType));
        env.bindTypeCon("Array", VirgilArray.TYPECON);
        // bind the raw types.
        ...
        V3System system = new V3System(program);
        VirgilComponent systemC = system.component;
        env.bindTypeCon(systemC.getName(), systemC.getTypeCon());
        program.virgil.components.add(systemC.getName(), systemC);
        program.namespace.addDecl(systemC.getName(), systemC);
    }
